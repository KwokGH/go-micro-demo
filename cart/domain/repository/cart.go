...
import (
	"errors"
	"github.com/Kwok/microdemo/cart/domain/model"
	"gorm.io/gorm"
)
...
func (u *CartRepository) InitTable() error {
	return u.mysqlDb.AutoMigrate(&model.Cart{})
}
...
// UpdateCart 更新Cart信息
func (u *CartRepository) UpdateCart(carts []*model.Cart) error {
	return u.mysqlDb.Model(&model.Cart{}).Updates(carts).Error
}
